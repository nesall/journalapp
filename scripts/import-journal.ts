...
async function processImage(guid: string): Promise<{ key: string; thumbnailKey: string } | null> {
    ...
    if (existsSync(jpegPath)) {
        inputBuffer = await readFile(jpegPath);
    } else if (existsSync(heicPath)) {
        inputBuffer = await readFile(heicPath);
    } else if (existsSync(heicPathLower)) {
        inputBuffer = await readFile(heicPathLower);
    }
    ...
    // convert to jpeg (handles both jpeg and heic input)
